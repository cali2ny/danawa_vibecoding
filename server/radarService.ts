...
export async function getRadarData(month: string, nation: Nation): Promise<RadarData> {
  let cached = await storage.getRadarData(month, nation);
  if (cached) {
    return cached;
  }
  ...
  await storage.setRadarData(data);
  return data;
}
